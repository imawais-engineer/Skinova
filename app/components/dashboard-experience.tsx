...
export function DashboardExperience({ userName }: { userName: string }) {
  ...
  return (
    <div className="page-stack">
      ...
      <div className="section-grid xl:grid-cols-[minmax(0,1.25fr)_minmax(320px,0.75fr)]">
        <Panel className="gradient-border">
          <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
            <div>
              <StatusBadge tone="mint">Your care journey</StatusBadge>
              <h2 className="mt-5 text-2xl font-semibold text-white">Selfie to action plan in under three minutes</h2>
            ...
            <Link
              href="/results"
              className="inline-flex h-11 shrink-0 items-center justify-center rounded-xl border border-white/10 bg-white/5 px-5 text-sm font-semibold text-white transition hover:bg-white/10"
            >
              View results
          ...
          <div className="mt-10 grid gap-4 md:grid-cols-4">
            {careTimeline.map((step, index) => (
              <div key={step.label} className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
                ...
                <p className="mt-5 text-sm font-semibold text-white">{step.label}</p>
                <p className="mt-3 text-xs leading-6 text-slate-400">{step.value}</p>
              </div>
            ))}
          </div>
        </Panel>
          ...
          {analysis ? (
            <>
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="mt-3 text-4xl font-semibold text-white">{analysis.overallScore}%</p>
                </div>
                <StatusBadge tone={session?.mode === "live" ? "mint" : "cyan"}>
                  {session?.mode === "live" ? "Live scan" : "Demo scan"}
                </StatusBadge>
              </div>
              <p className="mt-5 text-sm leading-7 text-slate-300">{analysis.summary}</p>
              ...
              <Link
                href="/routine"
                className="mt-8 inline-flex h-11 w-full items-center justify-center rounded-xl bg-cyan-300 px-4 text-sm font-semibold text-slate-950 transition hover:bg-cyan-200"
              >
                Open routine
              </Link>
            </>
            ...
            <>
              <div>
                ...
                <p className="mt-3 text-2xl font-semibold text-white">Run your first skin scan</p>
              </div>
      ...
      <div className="section-grid lg:grid-cols-4">
        {experienceHighlights.map((item) => {
          ...
          return (
            <Panel key={item.label}>
              ...
              <p className="mt-3 text-xs leading-6 text-slate-400">{item.value}</p>
            </Panel>
          );
        })}
      </div>
